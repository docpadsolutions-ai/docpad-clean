-- Restore explicit p_hospital_id (avoids NULL hospital_id / auth_org mismatch → 23502).

drop function if exists public.create_vendor(uuid, text, text, text, text, text, text, text, text, text, text, text, integer, jsonb);
drop function if exists public.create_vendor(text, text, text, text, jsonb, text, text, integer, jsonb);
...
comment on function public.create_vendor(uuid, text, text, text, text, jsonb, text, text, integer, jsonb) is
  'Insert pharmacy_vendors; p_address {line1,line2,city,state,pincode}; admin for p_hospital_id only.';

revoke all on function public.create_vendor(uuid, text, text, text, text, jsonb, text, text, integer, jsonb) from public;
grant execute on function public.create_vendor(uuid, text, text, text, text, jsonb, text, text, integer, jsonb)
