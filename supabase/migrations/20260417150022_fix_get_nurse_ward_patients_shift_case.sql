...
-- This is the SQL the database records as having actually run, on 20260417150022.

CREATE OR REPLACE FUNCTION public.get_nurse_ward_patients(p_nurse_id uuid, p_hospital_id uuid, p_shift text, p_date date)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
begin
  with cand as (
    ...
    where wsa.practitioner_id = p_nurse_id
      ...
      -- FIXED: case-insensitive shift match; 'general' assignments always show regardless of selected shift
      and (
        lower(trim(wsa.shift)) = 'general'
        or p_shift is null
        or lower(trim(wsa.shift)) = lower(trim(p_shift))
      )
  )
