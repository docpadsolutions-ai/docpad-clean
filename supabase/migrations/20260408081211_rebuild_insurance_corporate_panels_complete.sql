...
-- This is the SQL the database records as having actually run, on 20260408081211.

-- Complete rebuild of insurance_corporate_panels view with all expected columns
...
SELECT 
  ...
  cp.type,
  cp.status,
  cp.status as is_active,
  cp.discount_percent,
  cp.credit_limit,
  cp.credit_period_days,
  cp.requires_preauth,
  cp.preauth_mandatory_above_amount,
  CONCAT('AGR-', LPAD(SUBSTR(cp.id::text, 1, 8), 8, '0')) as agreement_reference,
  COALESCE(
    (SELECT SUM(ic.claimed_amount) 
     ...
     AND ic.hospital_id = cp.hospital_id), 
    0
  ...
  CASE 
    WHEN cp.agreement_end_date < CURRENT_DATE THEN true 
    ELSE false 
  END as is_expired,
  cp.hospital_id,
  cp.created_at,
