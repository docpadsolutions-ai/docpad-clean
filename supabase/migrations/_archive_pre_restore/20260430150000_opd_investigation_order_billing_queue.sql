...
-- Legacy inserts with billing_status = unbilled: create charge row only; reception bills later (no rolling invoice here).
...
-- Avoid duplicate service_request charge rows when trg_fn_investigation_to_billing already created one; prefer master price columns.
