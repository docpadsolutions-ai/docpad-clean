...
-- This is the SQL the database records as having actually run, on 20260414142114.
...
BEGIN
  ...
  -- Mark bed as reserved (pending deposit collection)
  IF p_bed_id IS NOT NULL THEN
    UPDATE public.ipd_beds SET status = 'reserved', updated_at = now()
    WHERE id = p_bed_id;
  END IF;
