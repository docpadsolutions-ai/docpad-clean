...
function rowTone(days: number | null): string {
  ...
  if (days > 30) return "bg-red-50/80";
  if (days > 7) return "bg-amber-50/80";
  return "";
}
  ...
  const columns = useMemo<ColumnDef<OutstandingInvoiceRow>[]>(
    () => [
      ...
      },
      {
        ...
        header: "Patient",
        cell: ({ getValue }) => <span className="text-slate-700">{String(getValue() ?? "—")}</span>,
      },
      {
        accessorKey: "total_gross",
        header: () => <span className="text-right tabular-nums">Gross</span>,
        cell: ({ getValue }) => (
          <div className="text-right tabular-nums text-slate-800">{formatInr(n(getValue()))}</div>
        ),
      },
      {
      ...
      },
      {
        ...
        cell: ({ getValue }) => {
          ...
          return (
            <div className={`text-center tabular-nums ${d > 30 ? "font-semibold text-red-700" : d > 7 ? "font-medium text-amber-800" : "text-slate-600"}`}>
              {d <= 0 ? "—" : d}
            </div>
          );
        },
      },
      {
        ...
        cell: ({ row }) => (
          <button
            type="button"
            className="rounded-lg border border-slate-200 bg-white px-2.5 py-1 text-xs font-semibold text-blue-700 shadow-sm transition hover:bg-slate-50"
            onClick={() => {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-200 px-4 py-3">
        <h2 className="text-sm font-semibold text-slate-700">Outstanding invoices</h2>
        <p className="mt-0.5 text-xs text-slate-500">Balance due &gt; 0, sorted by due date (up to 500).</p>
      </div>
          ...
          <thead>
            {table.getHeaderGroups().map((hg) => (
              <tr key={hg.id} className="border-b border-slate-200 bg-slate-50">
                {hg.headers.map((h) => (
                  <th key={h.id} className="px-3 py-2 text-xs font-semibold uppercase tracking-wide text-slate-500">
          ...
          <tbody>
            {table.getRowModel().rows.length === 0 ? (
              ...
              table.getRowModel().rows.map((row) => {
                ...
                return (
                  <tr key={row.id} className={`border-b border-slate-100 ${tone}`}>
